...
export default function UserPredictionsDropdown({ userId, isExpanded, allowedStages }: UserPredictionsDropdownProps) {
        ...
        return (
            <div className="p-4 bg-gray-50/50 border-t border-gray-100 space-y-3">
                {[...Array(3)].map((_, i) => (
                    <div
                        key={i}
                        className="animate-pulse bg-white border border-gray-150 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 shadow-xs"
                    >
    ...
    if (predictions.length === 0) {
        return (
            <div className="p-8 text-center text-gray-500 bg-gray-50/50 border-t border-gray-100 flex flex-col items-center justify-center gap-2">
                ...
                <p className="text-xs text-gray-400">Only started or completed match predictions will appear here.</p>
            </div>
        );
    }
    ...
    const getStatusBadge = (status: string, kickOff: string) => {
        if (status === "FINISHED") {
            return (
                <span className="text-[10px] bg-gray-100 text-gray-600 font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider">
                    Finished
                </span>
            );
        }
        if (status === "LIVE") {
            return (
                <span className="text-[10px] bg-red-100 text-red-600 font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1">
    ...
    const getPointsBadge = (points: number, status: string) => {
        if (status !== "FINISHED" && status !== "LIVE") {
            return (
                <span className="text-xs font-bold bg-amber-55 bg-indigo-50 text-indigo-700 border border-indigo-100 px-2 py-1 rounded-lg">
                    Pending
                </span>
            );
        }
        ...
        if (points >= 13) {
            return (
                <span className="text-xs font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 px-2.5 py-1 rounded-lg flex items-center gap-1 shadow-xs">
        ...
        if (points > 0) {
            return (
                <span className="text-xs font-bold bg-indigo-50 text-indigo-700 border border-indigo-100 px-2.5 py-1 rounded-lg">
                    +{points} PTS
                </span>
            );
        }
        return (
            <span className="text-xs font-semibold bg-gray-50 text-gray-400 border border-gray-100 px-2.5 py-1 rounded-lg">
                0 PTS
            </span>
        );
    };
                    ...
                    >
                        {currentPredictions.map((pred: any) => {
                            ...
                            return (
                                <div
                                    key={pred._id}
                                    className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 hover:shadow-xs transition-shadow duration-200"
                                >
                                    ...
                                    <div className="flex flex-col gap-1 md:w-[180px] shrink-0">
                                        ...
                                        <div className="flex items-center text-gray-500 text-xs mt-1">
                                            ...
                                            <Clock className="w-3.5 h-3.5 ml-2.5 mr-1 text-gray-400" />
                                    ...
                                    <div className="flex items-center justify-center flex-1 py-2 px-3 bg-gray-50 rounded-xl border border-gray-100">
                                        <div className="flex items-center justify-between w-full max-w-sm gap-2 text-xs sm:text-sm">
                                            ...
                                            <div className="flex items-center justify-end flex-1 gap-2 text-right">
                                                <span className="font-semibold text-gray-700 truncate max-w-[80px] sm:max-w-none">
                                                    {match.homeTeam?.name}
                                                </span>
                                                {match.homeTeam?.flagUrl && (
                                                    <img
                                                        ...
                                                        className="w-7 h-5 object-cover rounded shadow-2xs border border-gray-200 shrink-0"
                                                    />
                                                )}
                                            </div>

                                            {/* Scores Display */}
                                            <div className="flex flex-col items-center shrink-0 min-w-[100px] border-x border-gray-200/60 px-4">
                                                {/* Actual Score */}
                                                ...
                                                {/* Predicted Score */}
                                                <div className="text-[10px] text-gray-500 font-bold bg-indigo-50/50 text-indigo-700/80 px-1.5 py-0.5 rounded-md mt-1 border border-indigo-100/50">
                                    ...
                                    <div className="flex flex-row md:flex-col items-center justify-between md:justify-center md:items-end gap-2 md:w-[130px] shrink-0 border-t md:border-t-0 pt-2 md:pt-0 border-gray-100">
                                        ...
                                        {isKnockout && (pred.penaltyPrediction || (isFinished && match.wentToPenalties)) && (
                                            <div className="text-[10px] flex flex-col items-start md:items-end gap-0.5">
                                                {pred.penaltyPrediction && (
                                                    <div className="text-purple-600 bg-purple-50 px-2 py-0.5 rounded border border-purple-100/80 font-bold tracking-wide">
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-between bg-white border border-gray-200 rounded-xl p-3 shadow-2xs">
                    <button
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-gray-200 text-xs font-semibold text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                    >
