...
function getStageTheme(stage: string) {
  ...
  if (s.includes("final") && !s.includes("quarter") && !s.includes("semi")) {
    ...
    return {
      ...
      badgeClass:
        "bg-gradient-to-r from-red-600 via-orange-500 to-yellow-500 text-white animate-pulse font-black border border-yellow-400/50",
    ...
    return {
      ...
      badgeClass:
        "bg-gradient-to-r from-red-600 to-orange-600 text-white font-black border border-orange-500/40",
      title: "Semi Final",
    };
    ...
    return {
      ...
      badgeClass:
        "bg-gradient-to-r from-amber-600 via-amber-500 to-yellow-500 text-white font-black border border-yellow-450/40",
    ...
    return {
      ...
      badgeClass:
        "bg-gradient-to-r from-orange-600 to-amber-500 text-white font-black border border-amber-500/30",
      title: "Quarter Final",
    };
    ...
    return {
      ...
      badgeClass:
        "bg-gradient-to-r from-amber-600 to-yellow-500 text-white font-black border border-yellow-500/20",
      title: stage,
    };
  } else {
    // Group Stage
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
        Welcome, {(user as any).nickname || user.name}
      </h1>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-slate-800 flex flex-col justify-between min-h-[148px]">
          ...
          <div className="mt-2.5">
            <Link
              ...
              className="text-xs text-indigo-600 dark:text-cyan-400 hover:underline font-semibold flex items-center gap-1"
            >
              Manage Groups &rarr;
            </Link>
          </div>
        ...
        <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between sm:items-center gap-3 bg-gray-50/50 dark:bg-slate-900/50">
          ...
          {heroPredictorData.match && (
            ...
            <div className="flex items-center gap-3 bg-slate-105 dark:bg-slate-800/80 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700/50 self-start sm:self-auto">
              <span className="text-xs font-bold text-gray-705 dark:text-slate-300">
                Last Match:
              </span>
              <div className="flex items-center gap-1.5 text-xs font-black text-gray-900 dark:text-white">
                ...
                {heroPredictorData.match.homeTeam?.flagUrl && (
                  <img
                    src={heroPredictorData.match.homeTeam.flagUrl}
                    alt=""
                ...
                <span className="bg-slate-200 dark:bg-slate-700 px-1.5 py-0.5 rounded font-mono text-[11px]">
                  {heroPredictorData.match.homeScore} -{" "}
                  {heroPredictorData.match.awayScore}
                </span>
                {heroPredictorData.match.awayTeam?.flagUrl && (
                  <img
                    src={heroPredictorData.match.awayTeam.flagUrl}
                    alt=""
                    className="w-4 h-3 object-cover rounded-xs"
                  />
                )}
        ...
        <div className="p-6 bg-gray-50/10 dark:bg-slate-900/20">
          {!heroPredictorData.match ? (
            <div className="text-center text-gray-500 dark:text-slate-400 py-6">
              No matches finished yet. Once the action starts, the top
              predictors for the last match will be featured here!
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {heroPredictorData.predictors.map(
                (predictor: any, index: number) => {
                  const getRankBadge = (idx: number) => {
                    if (idx === 0)
                      return (
                        <div className="absolute -top-2 -left-2 bg-yellow-500 text-white rounded-full p-1 shadow-md animate-bounce">
                          <Trophy className="w-4 h-4" />
                        </div>
                      );
                    if (idx === 1)
                      return (
                        <div className="absolute -top-2 -left-2 bg-slate-300 text-slate-800 rounded-full p-1 shadow-md">
                          <Medal className="w-4 h-4" />
                        </div>
                      );
                    if (idx === 2)
                      return (
                        <div className="absolute -top-2 -left-2 bg-amber-600 text-white rounded-full p-1 shadow-md">
                          <Medal className="w-4 h-4" />
                        </div>
                      );
                    return (
                      ...
                      <div className="flex flex-col items-center text-center mt-2 w-full">
                        ...
                        <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1 font-medium bg-slate-100 dark:bg-slate-900 px-2 py-0.5 rounded-full border border-slate-200/40 dark:border-slate-800/40">
                          ...
                          <span className="font-bold font-mono">
                            {predictor.homeScore}-{predictor.awayScore}
                          </span>
                        </p>
                      </div>
        ...
        <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50/50 dark:bg-slate-900/50">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            ...
            {nextMatches.length > 1
              ? "Featured Matchups Today"
              : "Featured Next Match"}
          </h3>
          <Link
            href="/dashboard/matches"
            className="text-sm text-indigo-600 dark:text-cyan-400 hover:underline font-semibold flex items-center gap-1"
          >
            View Schedule &rarr;
          </Link>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 w-full max-w-4xl flex flex-row items-center justify-between gap-2 sm:gap-6 md:gap-12">
                    {/* Home Team Hero Card */}
                    <div
                      className="flex-1 flex flex-col items-center animate-slide-left opacity-0"
                    ...
                    >
                      ...
                      >
                        <div className="fiery-border-content relative w-full h-full overflow-hidden rounded-[calc(1.5rem-3px)]">
                          <img
                            src={
                              match.homeTeam?.championImageUrl ||
                              "/dummy-champion.png"
                            }
                          ...
                          <div className="absolute bottom-2 left-2 right-2 sm:bottom-4 sm:left-4 sm:right-4 flex items-center gap-1.5 sm:gap-3">
                            {match.homeTeam?.flagUrl && (
                              <img
                                ...
                                alt=""
                                className="w-5 h-3.5 sm:w-8 sm:h-6 object-cover rounded shadow-md border border-white/20"
                              />
                            )}
                            <div className="min-w-0">
                              <p className="text-white font-black text-[10px] sm:text-sm md:text-lg leading-tight line-clamp-1 truncate">
                                {match.homeTeam?.name}
                              </p>
                            </div>
                          </div>
                    ...
                    {/* VS & Match Info Panel */}
                    <div
                      className="flex flex-col items-center justify-center flex-shrink-0 z-20 animate-drop-in opacity-0"
                      style={{ animationDelay: `${index * 150 + 100}ms` }}
                    >
                      <div
                        className={`${stageTheme.badgeClass} px-2 py-0.5 sm:px-6 sm:py-1.5 rounded-full text-[8px] sm:text-xs uppercase tracking-widest shadow-lg shadow-red-500/20 mb-2 sm:mb-4 whitespace-nowrap`}
                      >
                        {stageTheme.title}
                      </div>
                    ...
                    {/* Away Team Hero Card */}
                    <div
                      className="flex-1 flex flex-col items-center animate-slide-right opacity-0"
                    ...
                    >
                      ...
                      >
                        <div className="fiery-border-content relative w-full h-full overflow-hidden rounded-[calc(1.5rem-3px)]">
                          <img
                            src={
                              match.awayTeam?.championImageUrl ||
                              "/dummy-champion.png"
                            }
